...
**要求**: 支撑高并发用户量  
**方法**: JMeter 1000 线程 × 5 循环，ramp-up 5s，直连 feed 服务 port 8083。使用 Spring Boot 默认连接池（Tomcat=200, HikariCP=10, Lettuce=8）。
...
| 指标 | 扩容池 (Tomcat=1000/Hikari=200/Lettuce=200) 800线程 | 默认池 800线程 | 默认池 **1000线程** |
|------|:---:|:---:|:---:|
| 平均 | 13ms | 72ms | **184ms** |
| P50 | 8ms | 8ms | **151ms** |
| P90 | ≤20ms | 107ms | **223ms** |
| P95 | ≤42ms | 222ms | **947ms** |
| P99 | ≤92ms | 1156ms | **1904ms** |
| 吞吐量 | 864 req/s | 858 req/s | **1023 req/s** |

- **结论**: ✅ 1000 线程并发下零错误。Min=4ms 说明查询本身极快——P50=151ms 是默认连接池（HikariCP=10）在更大并发下的排队效应。1000 线程把 P99 推到 1904ms，扩容连接池可有效改善尾延迟。
