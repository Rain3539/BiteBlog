...
**要求**: 支撑高并发用户量  
**方法**: JMeter 800 线程 × 5 循环，ramp-up 5s，直连 feed 服务 port 8083。使用 Spring Boot 默认连接池（Tomcat=200, HikariCP=10, Lettuce=8）。
...
| 指标 | 扩容池 (Tomcat=1000/Hikari=200/Lettuce=200) | 默认池 (Tomcat=200/Hikari=10/Lettuce=8) |
|------|:---:|:---:|
| 平均 | 13ms | 72ms |
| P50 | 8ms | 8ms |
| P90 | ≤20ms | 107ms |
| P95 | ≤42ms | 222ms |
| P99 | ≤92ms | 1156ms |
| 吞吐量 | 864 req/s | 858 req/s |

- **结论**: ✅ 800 线程并发下零错误。P50 两次测试一致（8ms），说明查询本身不慢——瓶颈在排队等连接。扩容连接池后 P99 从 1156ms 降至 92ms，尾延迟改善 12 倍。
