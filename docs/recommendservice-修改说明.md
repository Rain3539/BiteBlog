...
本次实现定位为“可联调、可展示、可扩展”的课程设计版本：

- 标签召回优先使用 Elasticsearch `post_index`，ES 不可用或无结果时降级为 MySQL 关键词兜底。
- ItemCF 优先使用 Elasticsearch `item_sim_index` 查询物品相似度，ES 不可用或无结果时降级 Redis 相似池，再降级在线扫描 `user_behavior`。
- 曝光去重使用 Redis Lua 在推荐返回前原子筛选并预占，降低同一用户并发刷新时重复推荐概率。
- 冷启动使用预计算后的 Redis ZSet 热门池，Redis 不可用时降级到 MySQL 热门笔记。
- 预计算任务每 10 分钟重建一次 `recommend:hot:pool` 和 ES `item_sim_index`，同时提供内部接口手动触发，在线请求只做曝光过滤、轻量重排和作者打散。
...
| `service/RecommendSearchService.java` | ES 召回层，优先从 `post_index` 查询标签/关键词候选，从 `item_sim_index` 查询 ItemCF 相似候选 |
| `service/RecommendService.java` | 推荐核心逻辑，负责召回、排序、曝光过滤、冷启动和降级 |
| `service/RecommendPrecomputeService.java` | 轻量预计算任务，重建 Redis 热门池和 ES ItemCF 相似索引 |
...
| `sql/init-recommend-data.ps1` | 推荐测试数据脚本，生成 30 条推荐样例 |
...
- ES `item_sim_index`：ItemCF 相似笔记召回使用。
...
```text
优先读取 Redis ZSet recommend:hot:pool
-> 根据分数倒序取热门笔记
-> 过滤 exposure:{userId}
-> 补齐笔记详情
-> 返回推荐列表
...
如果 ES 查询失败或没有结果，服务会降级为 MySQL `LIKE` 召回，匹配 `title/content/shopName/address`，保证 ES 临时不可用时推荐接口仍可返回。
...
-> 写入 ES item_sim_index
```

在线请求时只根据用户最近交互过的 `noteId` 查询 `item_sim_index`，再把相似笔记加入候选池。

老用户推荐会读取当前用户近期行为：

```text
用户 A 交互过 note1、note2
-> 优先查询 Elasticsearch item_sim_index
-> 得到离线/预计算的相似笔记
-> ES 不可用或无结果时读取 Redis recommend:itemcf:similar:{noteId}
-> 作为 ItemCF 候选
```

如果 ES 和 Redis 中都没有预计算相似池，服务会降级为在线简化 ItemCF：
...
- ItemCF 相似度由预计算任务提前写入 ES `item_sim_index`，在线请求只按用户交互过的 `item_id` 查询 TopN 相似笔记。
- 冷启动优先读取预计算后的 Redis ZSet `recommend:hot:pool`，避免每次都查数据库排序。
...
- 曝光集合和热门池存储在 Redis 中，多实例部署时可以共享同一份推荐状态；ItemCF 预计算相似关系优先走 ES `item_sim_index`，Redis 相似池作为降级。
...
- `size > 50` 时截断为 50。
- Redis 曝光集合设置 TTL。
- 初始化脚本生成 30 条推荐样例，支持前端分页和加载更多演示。
...
- ES 标签召回、ES ItemCF 召回、Redis/MySQL 兜底分层实现，后续调整召回源不影响 Controller 和前端接口结构。
...
- 共生成 30 条 `Recommend Test` 笔记，用于分页和加载更多。
- 写入 `recommend:hot:pool`，用于冷启动。
- 写入 `recommend:itemcf:similar:{postId}`，作为 ItemCF 的 Redis 降级数据。
- 写入少量 `user_behavior`，用于个性化和在线 ItemCF 降级。
- 尝试同步样例笔记到 ES `post_index`，同步相似关系到 ES `item_sim_index`；ES 不可用时脚本会给出 warning，不影响 MySQL/Redis 数据初始化。
...
| ItemCF | ES `item_sim_index` 优先，Redis 相似池和在线 `user_behavior` 降级 | 符合 ES 召回要求，同时保留可用性兜底 |
...
| 相似度生成 | 定时任务和手动接口重建 ES `item_sim_index` | 后续可接入 MQ 做增量更新 |
