...
- 消费 RabbitMQ `note.published`、`note.deleted` 和 `interaction.*` 事件，准实时维护推荐侧 ES `post_index`、ES `item_sim_index` 和 Redis 热门池。
...
| `biteblog.interaction` | `interaction.*` | `recommend.interaction.queue` | 刷新该笔记热度，并重建 ES `item_sim_index` |
...
- 曝光集合、行为画像缓存和热榜存储在 Redis 中，多实例部署时可以共享同一份推荐状态；ItemCF 预计算相似关系优先走 ES `item_sim_index`，Redis 相似池作为降级。
...
| 相似度生成 | 定时任务、手动接口和 MQ 互动事件重建 ES `item_sim_index` | 当前为轻量级全量重建，后续可改成真正增量更新 |
