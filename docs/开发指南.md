...
| Maven | 3.8+ | 后端构建 |
| Node.js | 18+ | 前端构建 |
| Docker + Docker Compose | 最新版 | 中间件部署 |
| Git | 任意 | 版本控制 |
...
预期输出：两个用户注册成功，返回 userId 和 token。
...
- **包结构**：`com.biteblog.{service}.{controller/service/mapper/entity}`
- **统一响应**：所有接口返回 `Result<T>`，不要直接返回业务对象
- **异常处理**：业务异常用 `BusinessException(ErrorCode.XXX)`，不要手动构造 Result.fail
