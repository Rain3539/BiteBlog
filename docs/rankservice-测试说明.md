...
| 类别 | 要求 | 实现/验证 |
|------|------|-----------|
| 并发 | 热榜接口在 100 线程以上并发访问下稳定返回、零错误，并记录真实并发响应时间 | Redis ZSet 读榜；`F-1` 为单请求基线，`F-2` 为 100 线程并发压测 |
...
| F-2 | JMeter 100 线程并发压测 | 并发/性能 | `rank-service-test.jmx` 配置 100 threads × 5 loops，压测 Top10/分页两个读接口；响应时间取并发运行生成的 `rank-service-result.jtl` | **通过（100 线程/0 错误）** |
...
**方法**: 使用 `jmeter/rank-service-test.jmx`，Thread Group 配置为 `100` threads、`1s` ramp-up、`5` loops。并发项验证高频读路径，因此只启用 `GET /rank/top10` 与 `GET /rank/list` 两个读接口；响应时间取该 100 线程并发运行生成的 `jmeter/rank-service-result.jtl` 中 `elapsed/Latency` 聚合结果，不能使用 `F-1` 的单请求基线数据替代。为避免空榜导致每次读请求都触发 `ensureCache()` 重建，压测前先写入 20 条已发布测试笔记，并执行 `POST /rank/rebuild?type=daily`、`weekly`、`all` 预热 Redis ZSet。

| 配置项 | 值 |
|--------|----|
| ThreadGroup.num_threads | 100 |
| LoopController.loops | 5 |
| ThreadGroup.ramp_time | 1s |
| Sampler 数量 | 2 |
| 预期总请求数 | 100 × 5 × 2 = 1000 |
...
|------|------|
| 样本数 | 1000 |
| Max allThreads / grpThreads | 100 / 100 |
| Error% | 0.00% |
| 总平均响应时间 | 744.55ms |
| 总 P90 / P95 / P99 | 816ms / 837ms / 855ms |
| 吞吐量 | 128.27 req/s |

| Sampler | Count | Avg | Min | Max | P90 | P95 | P99 |
|---------|-------|-----|-----|-----|-----|-----|-----|
| `GET /rank/top10?type=daily` | 500 | 761.57ms | 556ms | 888ms | 830ms | 841ms | 857ms |
| `GET /rank/list?type=weekly&page=1&size=10` | 500 | 727.54ms | 221ms | 861ms | 801ms | 831ms | 841ms |

- **结论**: 已按 100 线程真实并发重跑，`rank-service-result.jtl` 中 `allThreads/grpThreads` 最大值均达到 100，且 1000 个读请求全部成功。该结果为并发运行数据，不使用 `F-1` 的单请求平均值，也不再使用旧 10 线程结果。
