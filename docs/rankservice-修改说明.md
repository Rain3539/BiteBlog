...
- 榜单展示摘要同步缓存到 `rank:item:{postId}`，读榜时使用 Redis `multiGet` 批量读取标题、店名、互动数等字段，避免 500 线程读压测时每次请求都回 MySQL `selectBatchIds`。
- 单次分页 `size` 限制为 1 ~ 50，单榜缓存最多保留 200 条，避免大分页和无限增长。
- 重建接口用于初始化和运维恢复，日常高频查询不依赖实时 SQL 排序。
- JMeter 压测脚本 `rank-service-test.jmx` 的线程数调整为 500，循环 25 次，并发项只启用 `GET /rank/top10` 和 `GET /rank/list` 两个高频读接口；响应时间统计以 500 线程并发运行生成的 JTL 为准，不使用单请求连续调用结果替代并发结论。

**对应测试编号**：`F-1`、`F-2`。
其中 `F-1` 是 Top10 单请求基线，`F-2` 是 500 线程并发压测口径。已重新生成 `jmeter/rank-service-result.jtl` 和 `jmeter/rank-service截图.png`：样本数 25000，`allThreads/grpThreads` 最大值 500/500，错误率 0.00%，总平均响应时间 842.35ms，P95 为 1054ms。旧 10/200/800 线程结果不再作为并发验收数据。
