...
- JMeter 压测脚本 `rank-service-test.jmx` 的线程数调整为 200，循环 25 次，并发项只启用 `GET /rank/top10` 和 `GET /rank/list` 两个高频读接口；响应时间统计以 200 线程并发运行生成的 JTL 为准，不使用单请求连续调用结果替代并发结论。

**对应测试编号**：`F-1`、`F-2`。
其中 `F-1` 是 Top10 单请求基线，`F-2` 是 200 线程并发压测口径。已重新生成 `jmeter/rank-service-result.jtl` 和 `jmeter/rank-service截图.png`：样本数 10000，`allThreads/grpThreads` 最大值 200/200，错误率 0.00%，总平均响应时间 1159.65ms，P95 为 1411ms。旧 10 线程结果不再作为并发验收数据。
